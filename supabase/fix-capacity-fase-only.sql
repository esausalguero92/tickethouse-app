-- =============================================================
-- Capacidad manejada SOLO en fase (tier_phases)
-- - Quita check de capacity del evento
-- - Quita check de capacity del tier
-- - Solo verifica capacity de la fase activa (con FOR UPDATE)
-- - events.tickets_sold lo maneja el trigger trg_tickets_sold
--   (no se duplica con UPDATE manual)
...
BEGIN
  -- Idempotencia: si ya existen tickets para esta orden, retornar
  ...
  -- Cargar evento solo para prefix y contador correlativo
  SELECT e.id, e.tickets_sold, e.code_prefix
    INTO v_event
  ...
  -- Verificar capacidad SOLO por fase (con FOR UPDATE — protección real contra concurrencia)
    ...
    LOOP
      IF (v_tier_item->>'phase_id') IS NOT NULL THEN
        ...
        IF FOUND AND v_phase.capacity IS NOT NULL
          ...
          RETURN jsonb_build_object('error', 'Agotada la fase de ' || v_phase.name);
        END IF;
      END IF;
    END LOOP;
  END IF;

  -- Insertar tickets (el trigger trg_tickets_sold actualiza events.tickets_sold automáticamente)
  ...
  -- Actualizar tickets_sold por tier y por fase (para visualización en admin)
  IF v_order.tier_items IS NOT NULL THEN
    FOR v_tier_item IN SELECT value FROM jsonb_array_elements(v_order.tier_items) AS value
    LOOP
  ...
  -- events.tickets_sold lo actualiza el trigger automáticamente (NO se toca aquí)
