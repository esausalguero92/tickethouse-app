...
async function generateTicketPdf({ publicCode, correlativeCode, qrToken, eventName, eventDate, eventVenue, buyerName, locationUrl }) {
  ...
  try {
    ...
    tmpDoc.font('Bebas'); // necesario: heightOfString usa la fuente activa del doc, no la de options
    nameHeightEst = tmpDoc.heightOfString(eventName.toUpperCase(), {
      fontSize: 36, width: W - 48, characterSpacing: 2, lineGap: 2,
    });
  ...
  // Calcular contentBottom dinámicamente basado en locBoxH real
  // code box: yCodeRef + 40 (codeBoxH=46, empieza en y-6, termina en y+40)
  // location box: + 6 gap + locBoxH
  const locBoxH = 46;
  const contentBottom = cleanLocationUrl
    ? yCodeRef + 40 + 6 + locBoxH   // 40 + 6 + 46 = 92
    : yCodeRef + 40;
  // H siempre dinámico: 22px gap antes del footer + 26px footer + 30px margen de seguridad
  const H = contentBottom + 22 + 26 + 30;
  ...
  return new Promise((resolve, reject) => {
    ...
    if (cleanLocationUrl) {
      ...
      const locBoxH = 46;     // mismo alto que el code box

      // Fondo oscuro — igual al code box
      doc.rect(44, locY, W - 88, locBoxH).fill(C.surface);

      // Bordes accent en los 4 lados (borde completo, como botón)
      doc.rect(44,              locY,                  W - 88, 1).fill(C.accent); // top
      doc.rect(44,              locY + locBoxH - 1,    W - 88, 1).fill(C.accent); // bottom
      doc.rect(44,              locY,                  1, locBoxH).fill(C.accent); // left
      doc.rect(44 + (W - 88) - 1, locY,               1, locBoxH).fill(C.accent); // right

      // Label pequeño — mismo estilo que "CÓDIGO DE ACCESO"
      doc.font('Grotesk')
         .fontSize(7)
         .fillColor(C.muted)
         .text('UBICACIÓN DEL EVENTO', 0, locY + 8, {
           align: 'center', width: W, characterSpacing: 2.5,
         });

      // Texto principal en Bebas — mismo estilo que el código de acceso
      doc.font('Bebas')
         .fontSize(20)
         .fillColor(C.accent)
         .text('VER UBICACION EN MAPA', 0, locY + 20, {
           align: 'center', width: W, characterSpacing: 2,
         });
